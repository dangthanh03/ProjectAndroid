...
public class Login extends AppCompatActivity {
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            checkIfUserExistsInDatabase(user.getEmail());
    ...
    private void checkIfUserExistsInDatabase(String email) {
        DatabaseReference usersRef = FirebaseDatabase.getInstance().getReference("users");

        // Tìm kiếm người dùng theo email trong Realtime Database
        usersRef.orderByChild("email").equalTo(email)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onCancelled(@NonNull DatabaseError databaseError) {
                        ...
                        Toast.makeText(Login.this, "Error checking user in database.",
                                Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void createUserInDatabase(String email) {
        // Tạo người dùng mới với thông tin mặc định hoặc theo yêu cầu của bạn
        // Ở đây, tạo một người dùng với thông tin cơ bản và thêm vào Realtime Database
        FirebaseUser user = mAuth.getCurrentUser();
        if (user != null) {
            String userId = user.getUid();
            User newUser = new User(email, "defaultName", "defaultPhone", userId, "defaultFCM", "defaultImgUrl");
            DatabaseReference usersRef = FirebaseDatabase.getInstance().getReference("users");
            usersRef.child(userId).setValue(newUser)
                    ...
                    .addOnFailureListener(e -> {
                        Toast.makeText(Login.this, "Error creating user in database.",
                                Toast.LENGTH_SHORT).show();
                    });
        }
    }
